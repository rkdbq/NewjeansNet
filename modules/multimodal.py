...
class MultiModalModel(nn.Module):
    def __init__(self, text_input_dim, image_input_dim, hidden_dim, num_classes):
        ...
        self.text_fc1 = nn.Linear(100, hidden_dim).cuda()
        self.text_bn1 = nn.BatchNorm1d(hidden_dim*2).cuda()
        self.text_relu1 = nn.ReLU().cuda()
        self.text_fc2 = nn.Linear(hidden_dim, hidden_dim).cuda()
        self.text_bn2 = nn.BatchNorm1d(hidden_dim*2).cuda()
        self.text_relu2 = nn.ReLU().cuda()
        ...
        self.image_fc1 = nn.Linear(hidden_dim * 256 * 384, hidden_dim).cuda()
        self.image_bn1 = nn.BatchNorm1d(1).cuda()
        self.image_relu1 = nn.ReLU().cuda()
        self.image_fc2 = nn.Linear(hidden_dim, hidden_dim).cuda()
        self.image_bn2 = nn.BatchNorm1d(1).cuda()
        self.image_relu2 = nn.ReLU().cuda()
    ...
    def forward(self, text_input, image_input):
        ...
        text_output = self.text_fc1(text_embedded).squeeze(0)
        text_output = self.text_bn1(text_output)
        text_output = self.text_relu1(text_output)
        text_output = self.text_fc2(text_output)
        text_output = self.text_bn2(text_output)
        text_output = self.text_relu2(text_output)
        ...
        image_output = self.image_fc1(image_output).unsqueeze(1)
        image_output = self.image_bn1(image_output)
        image_output = self.image_relu1(image_output)
        image_output = self.image_fc2(image_output)
        image_output = self.image_bn2(image_output)
        image_output = self.image_relu2(image_output)
